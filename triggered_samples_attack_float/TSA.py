...
from triggered_samples_attack_float.utils import *
from models import vanilla_resnet
import numpy as np
import config
from models.model_wrap_float import Attacked_model
import copy
import argparse

parser = argparse.ArgumentParser(description='Triggered Samples Attack for Full-Precision Networks')
...
parser.add_argument('--masked-e-bits', '-masked_e_bits', default=5, type=int)
...
model = torch.nn.DataParallel(vanilla_resnet.resnet20_mid(class_num))


checkpoint = torch.load(os.path.join(config.float_model_root, "model.th"))
...
load_model = Attacked_model(model, "cifar10", "resnet20")
model.cuda()
load_model.cuda()
load_model.eval()
    ...
    for ext_iter in range(ext_max_iters):
        ...
        for inn_iter in range(inn_max_iters):
            ...
            for i, (input, target) in enumerate(aux_loader):
                ...
                attacked_model.w_twos.data[:, :, args.masked_e_bits+1:] = attacked_model.w_twos.data[:, :, args.masked_e_bits+1:] - \
                                                                       inn_lr_bit * attacked_model.w_twos.grad.data[:, :, args.masked_e_bits+1:]
                attacked_model.w_twos.data[:, :, 0] = attacked_model.w_twos.data[:, :, 0] - \
                                                      inn_lr_bit * attacked_model.w_twos.grad.data[:, :, 0]
