...
def bit2float_device(b, num_e_bits=8, num_m_bits=23, bias=127.):
    """Turn input tensor into float.
        Args:
            b : binary tensor. The last dimension of this tensor should be the
            the one the binary is at.
    ...
    s = torch.index_select(b, -1, torch.arange(0, 1).to(b.device))
    e = torch.index_select(b, -1, torch.arange(1, 1 + num_e_bits).to(b.device))
    m = torch.index_select(b, -1, torch.arange(1 + num_e_bits,
                                               1 + num_e_bits + num_m_bits).to(b.device))
    # SIGN BIT
    # out = ((-1) ** s).squeeze(-1).type(dtype)
    # out1 = ((-1) ** s).squeeze(-1).type(dtype)
    out1 = (1-2*s).squeeze(-1).type(dtype)

    # EXPONENT BIT
    exponents = -torch.arange(-(num_e_bits - 1.), 1.).to(b.device)
    exponents = exponents.repeat(b.shape[:-1] + (1,))
    e_decimal = torch.sum(e * 2 ** exponents, dim=-1) - bias
    # out *= 2 ** e_decimal
    out2 = out1 * (2 ** e_decimal)
    # MANTISSA
    matissa = (torch.Tensor([2.]).to(b.device) ** (
        -torch.arange(1., num_m_bits + 1.).to(b.device))).repeat(
        m.shape[:-1] + (1,))
    # out *= 1. + torch.sum(m * matissa, dim=-1)
    out3 = out2 * (1. + torch.sum(m * matissa, dim=-1))
    return out3
