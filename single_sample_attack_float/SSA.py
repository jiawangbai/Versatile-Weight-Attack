...
import copy
from single_sample_attack_float.utils import *
from single_sample_attack_float.convert import bit2float_device, float2bit

parser = argparse.ArgumentParser(description='Single Sample Attack for Full-Precision Networks')
...
parser.add_argument('--lam2', '-lam2', default=50, type=float)
parser.add_argument('--k', '-k', default=5, type=float)
parser.add_argument('--n-aux', '-n_aux', default=128, type=int)

parser.add_argument('--masked-e-bits', '-masked_e_bits', default=6, type=int)
...
class AugLag(nn.Module):
    def __init__(self, w, b, init=False):
        super(AugLag, self).__init__()


        self.b = nn.Parameter(torch.tensor(b).float(), requires_grad=True)

        self.w_twos = nn.Parameter(torch.zeros([w.shape[0], w.shape[1], 32]), requires_grad=True)
...
def attack(auglag_ori, all_data, labels, labels_cuda, clean_output,
    ...
    for ext_iter in range(ext_max_iters):
        ...
        for inn_iter in range(inn_max_iters):
            ...
            # update all bits except for the high-order ones, equivalent to the gradient mask defined in our paper
            auglag.w_twos.data[target_class][:, args.masked_e_bits+1:] = auglag.w_twos.data[target_class][:, args.masked_e_bits+1:] - \
                                               inn_lr * auglag.w_twos.grad.data[target_class][:, args.masked_e_bits+1:]
            auglag.w_twos.data[source_class][:, args.masked_e_bits+1:] = auglag.w_twos.data[source_class][:, args.masked_e_bits+1:] - \
                                               inn_lr * auglag.w_twos.grad.data[source_class][:, args.masked_e_bits+1:]
            auglag.w_twos.data[target_class][:, 0] = auglag.w_twos.data[target_class][:, 0] - \
                                                     inn_lr * auglag.w_twos.grad.data[target_class][:, 0]
            auglag.w_twos.data[source_class][:, 0] = auglag.w_twos.data[source_class][:, 0] - \
                                                     inn_lr * auglag.w_twos.grad.data[source_class][:, 0]
    ...
    pred = pred.squeeze(1)
    pa_acc = len([i for i in range(len(output)) if labels[i] == pred[i].item() and i != target_idx and i not in aux_idx]) / \
                     (len(labels) - 1 - n_aux)
...
def main():
    ...
    weight, bias = load_model(arch)
    all_data, labels = load_data(arch)
    labels_cuda = labels.cuda()

    auglag = AugLag(weight, bias, init=True).cuda()
