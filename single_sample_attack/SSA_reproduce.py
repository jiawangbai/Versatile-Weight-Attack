...
parser.add_argument('--init-lam2', '-init_lam2', default=100, type=float)
...
def main():
    ...
    # The 1,000 attacked samples used in our experiments for Single Sample Attack
    ...
    for i, (target_class, attack_idx) in enumerate(attack_info):
        ...
        for search_k in range(args.max_search_k):
            ...
            cur_lam2 = args.init_lam2

            for search_lam2 in range(args.max_search_lam):

                res = attack(auglag, all_data, labels, labels_cuda, clean_output,
                             attack_idx, target_class, source_class, aux_idx,
                             args.lam1, cur_lam2, cur_k, args)

                if res == -1:
                    print("Error[{0}]: Lambda:{1} K_bits:{2}".format(i, cur_lam2, cur_k))
                    cur_lam2 = cur_lam2 / 2.0
                    continue
                elif res["suc"]:
        ...
        if not suc:
            ...
            print("[{0}] PA-ACC:{1:.4f} Success:{2} N_flip:{3} Stop:{4} Lambda_2:{5} K:{6}".format(
